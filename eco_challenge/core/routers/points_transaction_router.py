...
@router.get('/')
async def get_points_transaction(storage: PointsTransactionStorageDepends) -> list[PointsTransactionGet]:
    return await storage.get_objects()


@router.get('/{points_transaction_id}')
async def get_one_points_transaction(points_transaction_id: int, storage: PointsTransactionStorageDepends):
